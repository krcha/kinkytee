...
class FileBird_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in FileBird_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The FileBird_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( 'njt-filebird-'.$this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/filebird-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		 ...
		 * class.
		 */
		wp_register_script( 'njt-filebird-upload-localize', plugin_dir_url( __FILE__ ) . 'js/filebird-util.js', array( 'jquery' ), $this->version, false );
		...
		wp_enqueue_script( 'njt-filebird-'.$this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/filebird-admin.js', array( 'jquery' ), $this->version, false );
	...
	public function nt_upload() {
		...
		wp_enqueue_script( 'njt-filebird-drag-scripts', plugin_dir_url( __FILE__ ) . 'plugin/drag/jquery.event.drag-2.2.js', array( 'jquery' ), $this->version, false );
		wp_enqueue_script( 'njt-filebird-drop-scripts', plugin_dir_url( __FILE__ ) . 'plugin/drop/jquery.event.drop-2.2.js', array( 'jquery' ), $this->version, false );
	...
	?>
		<div id="filebird_sidebar" style="display: none;">
			...
			<div class="filebird_sidebar panel-left"
				<?php echo ($sidebar_splitter_width ? ' style="width: '. $sidebar_splitter_width .'px;"' : '') ?>
			>
				...
				>
					...
					<div id="njt-filebird-defaultTree" class="filebird_tree">
						<ul>
							<li id="menu-item-all" data-jstree='{"selected":true}' id="menu-item-all" data-id="all" data-number="<?php echo $all_count; ?>" class="menu-item">
								<svg
 ...
 */
	...
	private function build_folder($folders)
	{
	    ...
	    foreach ($folders as $k => $v) {
	        ...
	        ?>
	        <li id="menu-item-<?php echo $v->term_id; ?>" data-id="<?php echo $v->term_id; ?>" <?php echo $v->count?  "data-number={$v->count}" : ''; ?> class="menu-item menu-item-depth-<?php echo $depth; ?>">
