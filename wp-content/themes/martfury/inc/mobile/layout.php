...
class Martfury_Mobile {
	...
	function __construct() {
		...
		add_filter( 'option_page_on_front', array( $this, 'homepage_mobile_init' ) );
	...
	function header_mobile() {
		...
		$layout = martfury_is_homepage() ? 'v1' : 'v2';
	...
	function mobile_modal_popup() {
		...
		$modal_class = $cat_header && ( ! intval( martfury_get_option( 'navigation_mobile' ) ) || ! $cat_navi ) ? 'not-cat-navigation' : '';

		echo sprintf( '<div class="mf-els-modal-mobile %s" id="mf-els-modal-mobile">', esc_attr( $modal_class ) );

		if ( $search_navi ) {
			get_template_part( 'template-parts/mobile/search' );
		}

		if ( $cat_navi || $cat_header ) {
